...
export type AudioMix = { duck: Duck; enhance: string; masterDb: number; loudnorm: boolean; trackGain: Record<string, number>; cleanMix: number };
...
export function blankComposition(): Composition {
  return {
    ...
    audio: { duck: { enabled: false, musicDb: -18, duckedDb: -30, attack: 0.02, release: 0.4 }, enhance: "neural", masterDb: 0, loudnorm: false, trackGain: {}, cleanMix: 1 },
...
export function normalize(raw: unknown, assets: Asset[] = []): Composition {
  ...
  return {
    ...
    audio: { ...b.audio, ...(r.audio ?? {}), duck: { ...b.audio.duck, ...(r.audio?.duck ?? {}) }, trackGain: (r.audio?.trackGain && typeof r.audio.trackGain === "object" ? r.audio.trackGain : {}), cleanMix: num((r.audio as any)?.cleanMix, 1) },
