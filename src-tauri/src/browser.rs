...
pub async fn webview_open(
    ...
    // Existing EMBEDDED child webview? reposition + navigate. get_webview finds
    // child webviews (not windows).
    ...
    // EMBED a child webview INSIDE the main window (NOT a separate window —
    // .parent() made a free-floating OS window that covered the UI). add_child
    // on the main WebviewWindow (which derefs to Window) clips it to the parent
    // window + positions it at the pane rect.
