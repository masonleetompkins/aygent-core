...
/// Lightweight list entry (no bodies) for the conversation switcher.
...
/// List conversation metadata for a folder, newest-first (by `updated`).
    ...
    for entry in rd.flatten() {
        ...
        if let Ok(text) = fs::read_to_string(&path) {
            if let Ok(c) = serde_json::from_str::<Conversation>(&text) {
                out.push(ConvMeta { id: c.id, title: c.title, updated: c.updated });
            }
        }
    }
...
/// Save (create or overwrite) a conversation. Stamps `updated` to now.
